    ...
    >
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>
            {isZh
              ? "新功能：独立 Agent 引擎 + OpenAI 支持"
              : "New: Independent Agent Engine + OpenAI Support"}
          </DialogTitle>
          <DialogDescription>
            {isZh
              ? "本次更新带来了底层架构变更"
              : "This update includes architectural changes"}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-3 text-sm">
          ...
          {isZh ? (
            <>
              ...
              <div className="space-y-2 text-muted-foreground">
                <p>
                  ...
                  — 无需安装 CLI，支持所有已配置的服务商
                </p>
                <p>
                  <span className="text-foreground font-medium">
                    Claude Code 引擎
                  </span>{" "}
                  — 通过 CLI 驱动，获得完整的命令行能力
                </p>
              </div>
              <p>
                同时支持 <span className="font-medium">OpenAI 授权登录</span>
                ，ChatGPT Plus/Pro 用户可在服务商设置中登录后直接使用 GPT-5.5
                等模型。
              </p>
            </>
          ) : (
            <>
              <p>
                CodePilot can now run independently without the Claude Code CLI.
              </p>
              <div className="space-y-2 text-muted-foreground">
                <p>
                  <span className="text-foreground font-medium">
                  ...
                  — no CLI needed, works with all configured providers
                </p>
                <p>
                  <span className="text-foreground font-medium">
                    Claude Code engine
                  </span>{" "}
                  — driven by CLI for full command-line capabilities
                </p>
              </div>
              <p>
                Also supports{" "}
                <span className="font-medium">OpenAI OAuth login</span> —
                ChatGPT Plus/Pro users can sign in under Providers to use
                GPT-5.5 and more.
              </p>
            </>
          )}
