...
async function testLayoutAndNav(context: BrowserContext) {
  ...
  record(
    ...
    sidebarVisible ? "Sidebar is visible" : "Sidebar not found",
  );
  ...
  const isActiveChat =
    chatNavClasses.includes("active") ||
    chatNavClasses.includes("bg-") ||
    chatNavClasses.includes("font-medium") ||
    chatNavClasses.includes("font-semibold");
  record(
    "Layout",
    "Chat nav highlighted on /chat",
    isActiveChat ? "PASS" : "FAIL",
    `Classes: ${chatNavClasses.substring(0, 100)}`,
  );
  ...
  record(
    "Layout",
    "Navigate to Plugins via nav",
    pluginsUrl.includes("/plugins") ? "PASS" : "FAIL",
    `URL: ${pluginsUrl}`,
  );
  ...
  record(
    ...
    settingsUrl.includes("/settings") ? "PASS" : "FAIL",
    `URL: ${settingsUrl}`,
  );
  ...
  if (themeToggleBtn) {
    ...
    const styleBefore =
      (await page.locator("html").getAttribute("style")) || "";
    ...
    record(
      ...
      `Before: class="${htmlBefore}" style="${styleBefore}" | After: class="${htmlAfter}" style="${styleAfter}"`,
    );

    // Take dark mode screenshot
    await page.screenshot({
      path: "/Users/erbanku/Documents/code/opus-4.6-test/src/__tests__/screenshots/dark-mode.png",
      fullPage: true,
    });
  ...
  const collapseBtn = page.locator(
    '[data-testid="sidebar-toggle"], button[aria-label*="sidebar"], button[aria-label*="collapse"], button[title*="sidebar"]',
  );
  ...
  if (collapseCount > 0) {
    ...
    await page.waitForTimeout(500);
    await page.screenshot({
      path: "/Users/erbanku/Documents/code/opus-4.6-test/src/__tests__/screenshots/sidebar-collapsed.png",
      fullPage: true,
    });
    record(
      "Layout",
    ...
    record(
      ...
      "SKIP",
      "No collapse button found with common selectors",
    );
  }
...
async function testMobileResponsive(context: BrowserContext) {
  ...
  await page.screenshot({
    path: "/Users/erbanku/Documents/code/opus-4.6-test/src/__tests__/screenshots/mobile-chat.png",
    fullPage: true,
  });
  ...
  const sidebarHidden =
    !sidebarBox || sidebarBox.width === 0 || sidebarBox.x < -100;
  record(
    "Responsive",
    "Sidebar hidden on mobile",
    sidebarHidden ? "PASS" : "FAIL",
    sidebarBox
      ? `Box: x=${sidebarBox.x} w=${sidebarBox.width}`
      : "No bounding box",
  );

  // Check for hamburger menu
  const hamburger = page.locator(
    'button[aria-label*="menu"], button[aria-label*="sidebar"], [data-testid="mobile-menu"]',
  );
  const hamburgerCount = await hamburger.count();
  record(
    "Responsive",
    "Hamburger menu button exists",
    hamburgerCount > 0 ? "PASS" : "FAIL",
    `Found ${hamburgerCount} potential hamburger buttons`,
  );
  ...
  await page.waitForTimeout(500);
  await page.screenshot({
    path: "/Users/erbanku/Documents/code/opus-4.6-test/src/__tests__/screenshots/mobile-plugins.png",
    fullPage: true,
  });
  record(
    "Responsive",
    "Plugins page renders on mobile",
    "PASS",
    "Screenshot taken",
  );
  ...
  await page.waitForTimeout(500);
  await page.screenshot({
    path: "/Users/erbanku/Documents/code/opus-4.6-test/src/__tests__/screenshots/mobile-settings.png",
    fullPage: true,
  });
  record(
    "Responsive",
    "Settings page renders on mobile",
    "PASS",
    "Screenshot taken",
  );
...
async function testChatFlow(context: BrowserContext) {
  ...
  record(
    ...
    newChatVisible ? "PASS" : "FAIL",
    newChatVisible ? "Found" : "Not found",
  );
  ...
  record(
    "Chat",
    "Chat input visible",
    inputVisible ? "PASS" : "FAIL",
    inputVisible ? "Found" : "Not found",
  );

  // --- Send button ---
  const sendBtn = page.locator(
    'button[type="submit"], button:has(svg[class*="send"]), button[aria-label*="send"]',
  );
  ...
  record(
    "Chat",
    "Send button present",
    sendBtnCount > 0 ? "PASS" : "FAIL",
    `Found ${sendBtnCount} potential send buttons`,
  );
  ...
  if (inputVisible) {
    ...
    record(
      ...
      placeholder.length > 0 ? "PASS" : "FAIL",
  ...
  record(
    ...
    hasEmptyState ? "Claude Chat welcome shown" : "Empty state not found",
  );
  ...
  if (inputVisible) {
    ...
    record(
      ...
      inputValue.includes("test message") ? "PASS" : "FAIL",
    ...
    const userMessage = page.locator(
      '[class*="message"], [data-role="user"], [class*="user"]',
    );
    const messageCount = await userMessage.count();
    record(
      "Chat",
      "Message appears after sending",
      messageCount > 0 ? "PASS" : "FAIL",
      `Found ${messageCount} message elements`,
    );

    // Take screenshot of the chat after sending
    await page.screenshot({
      path: "/Users/erbanku/Documents/code/opus-4.6-test/src/__tests__/screenshots/chat-after-send.png",
      fullPage: true,
    });
    ...
    record(
      "Chat",
      "URL updates after sending message",
      urlAfterSend !== "http://localhost:3000/chat" ? "PASS" : "FAIL",
      `URL: ${urlAfterSend}`,
    );
    ...
    const recentChats = page
      .locator('[class*="sidebar"] a, aside a')
      .filter({ hasNotText: "New Chat" })
      .filter({ hasNotText: "Chat" });
    const chatLinks = await page.locator('aside a[href*="/chat/"]').count();
    record(
      "Chat",
      "Conversation appears in sidebar",
      chatLinks > 0 ? "PASS" : "FAIL",
      `Found ${chatLinks} chat links in sidebar`,
    );
  }
...
async function testPlugins(context: BrowserContext) {
  ...
  const hasFilters =
    ...
    (await globalTab.isVisible().catch(() => false));
  record(
    "Plugins",
    "Filter tabs visible (All/Global/Project)",
    hasFilters ? "PASS" : "FAIL",
    hasFilters ? "Found" : "Not found",
  );
  ...
  record(
    "Plugins",
    "MCP Servers button/link visible",
    hasMcpLink ? "PASS" : "FAIL",
    hasMcpLink ? "Found" : "Not found",
  );
  ...
  if (hasMcpLink) {
    ...
    record(
      ...
      mcpUrl.includes("/plugins/mcp") ? "PASS" : "FAIL",
      `URL: ${mcpUrl}`,
    );
  }
...
async function testMCPPage(context: BrowserContext) {
  const page = await context.newPage();
  await page.goto("http://localhost:3000/plugins/mcp", {
    waitUntil: "networkidle",
  });
  await page.waitForTimeout(500);
  ...
  const hasTabs =
    (await serversTab.isVisible().catch(() => false)) &&
    (await jsonTab.isVisible().catch(() => false));
  record(
    "MCP",
    "Tabs visible (Servers/JSON Config)",
    hasTabs ? "PASS" : "FAIL",
    hasTabs ? "Found" : "Not found",
  );
  ...
  record(
    ...
    hasBackBtn ? "PASS" : "FAIL",
    hasBackBtn ? "Found" : "Not found",
  );
  ...
  if (hasAddBtn) {
    ...
    await page.waitForTimeout(500);
    await page.screenshot({
      path: "/Users/erbanku/Documents/code/opus-4.6-test/src/__tests__/screenshots/mcp-add-dialog.png",
      fullPage: true,
    });
    ...
    record(
      ...
      hasDialog ? "PASS" : "FAIL",
      hasDialog ? "Dialog visible" : "Dialog not found",
    );
    ...
    if (hasDialog) {
      ...
      const nameInput = page
        .locator(
          'input[placeholder*="name"], input[name="name"], label:has-text("Name") + input, label:has-text("Name") ~ input',
        )
        .first();
      const hasName = await nameInput.isVisible().catch(() => false);
      record(
        "MCP",
        "Add dialog has name field",
        hasName ? "PASS" : "FAIL",
  ...
  if (hasTabs) {
    ...
    await page.waitForTimeout(500);
    await page.screenshot({
      path: "/Users/erbanku/Documents/code/opus-4.6-test/src/__tests__/screenshots/mcp-json-config.png",
      fullPage: true,
    });

    const jsonEditor = page.locator(
      'textarea, [class*="editor"], [class*="json"], pre, code',
    );
    const jsonEditorCount = await jsonEditor.count();
    record(
      "MCP",
      "JSON Config tab shows editor",
      jsonEditorCount > 0 ? "PASS" : "FAIL",
      `Found ${jsonEditorCount} editor elements`,
    );
  }
...
async function testSettings(context: BrowserContext) {
  ...
  const hasEditorToggle =
    (await visualBtn.isVisible().catch(() => false)) &&
    (await jsonBtn.isVisible().catch(() => false));
  record(
    "Settings",
    "Editor mode toggle visible",
    hasEditorToggle ? "PASS" : "FAIL",
    hasEditorToggle ? "Found" : "Not found",
  );
  ...
  record(
    ...
    hasPermissions ? "Found" : "Not found",
  );
  ...
  record(
    "Settings",
    "Environment Variables section visible",
    hasEnv ? "PASS" : "FAIL",
    hasEnv ? "Found" : "Not found",
  );
  ...
  if (hasEditorToggle) {
    ...
    await page.waitForTimeout(500);
    await page.screenshot({
      path: "/Users/erbanku/Documents/code/opus-4.6-test/src/__tests__/screenshots/settings-json-mode.png",
      fullPage: true,
    });
    ...
    record(
      "Settings",
      "JSON editor textarea visible",
      jsonVisible ? "PASS" : "FAIL",
      jsonVisible ? "Found" : "Not found",
    );
    ...
    if (jsonVisible) {
      ...
      record(
        "Settings",
        "JSON editor contains valid JSON",
        isValidJson ? "PASS" : "FAIL",
        `Content length: ${jsonContent.length}, preview: ${jsonContent.substring(0, 100)}`,
      );
    }
...
async function testConsoleErrors(context: BrowserContext) {
  ...
  for (const route of routes) {
    ...
    record(
      "Console",
      `No JS errors on ${route}`,
      criticalErrors.length === 0 ? "PASS" : "FAIL",
      criticalErrors.length === 0
        ? "No critical errors"
        : `Errors: ${criticalErrors.join(" | ").substring(0, 200)}`,
    );
