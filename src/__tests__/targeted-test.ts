...
async function main() {
  ...
  console.log("=== Test 1: Mobile sidebar behavior ===");
  {
    ...
    await page.screenshot({
      path: "/Users/erbanku/Documents/code/opus-4.6-test/src/__tests__/screenshots/mobile-sidebar-closed.png",
    });
    ...
    console.log(
      ...
      await headerToggle.innerText().catch(() => "none"),
    );
    ...
    await page.waitForTimeout(500);
    await page.screenshot({
      path: "/Users/erbanku/Documents/code/opus-4.6-test/src/__tests__/screenshots/mobile-sidebar-reopened.png",
    });
  ...
  console.log("\n=== Test 2: Chat URL and sidebar update ===");
  {
    ...
    await page.screenshot({
      path: "/Users/erbanku/Documents/code/opus-4.6-test/src/__tests__/screenshots/chat-after-complete.png",
    });
  ...
  console.log("\n=== Test 4: MCP back navigation ===");
  {
    ...
    for (let i = 0; i < Math.min(backButtons.length, 10); i++) {
      ...
      console.log(
        `  [${i}] ${tag} text="${text}" aria-label="${ariaLabel}" href="${href}"`,
      );
    }

    // Look for the specific back button (arrow left icon that links to /plugins)
    const backLink = page
      .locator('a[href="/plugins"], button')
  ...
  console.log("\n=== Test 5: Sidebar collapse on desktop ===");
  {
    ...
    const headerToggle = page.locator("header button").first();
    const srText = await page
      .locator("header button .sr-only")
      .first()
      .innerText()
      .catch(() => "");
    ...
    console.log(
      ...
      sidebarBefore
        ? `x=${sidebarBefore.x} w=${sidebarBefore.width}`
        : "not found",
    );
    ...
    console.log(
      "Sidebar after toggle:",
      sidebarAfter
        ? `x=${sidebarAfter.x} w=${sidebarAfter.width}`
        : "hidden/not found",
    );

    await page.screenshot({
      path: "/Users/erbanku/Documents/code/opus-4.6-test/src/__tests__/screenshots/desktop-sidebar-collapsed.png",
    });
    ...
    console.log(
      "Sidebar after re-expand:",
      sidebarReopen
        ? `x=${sidebarReopen.x} w=${sidebarReopen.width}`
        : "hidden/not found",
    );
