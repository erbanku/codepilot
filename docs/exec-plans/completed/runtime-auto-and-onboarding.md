...
- `src/lib/claude-home-shadow.ts:234-319` `createShadowClaudeHome({ stripAuth })`——DB provider 模式剥离 ANTHROPIC\_\* env keys
...
- 推断：auto 模式在 cc-switch 用户 / 全新用户场景下**错选 Native**，Native 没有读 `~/.claude/settings.json` 的能力，遇到"DB 无 provider + env 无 ANTHROPIC\_\* + 用户未登录 CodePilot 但已装 cc-switch"时抛错
...
- 方案 B：**会**触发（注意：之前文档旧版写的"不触发"是**错的**，与 Phase B 自己的 `hasCodePilotProvider()` 契约矛盾——Codex 核查结论 2026-04-15 指出并修正）。Phase B 契约明确规定 `hasCodePilotProvider()` **不**读 `~/.claude/settings.json`，只看 DB provider / process.env ANTHROPIC\_\* / legacy `anthropic_auth_token` setting。纯 cc-switch 用户这三者都为空 → 会被入口拦截 → 弹 SetupCenter Provider 卡片。
     ...
     if (!response.ok) {
       ...
       if (err.code === "NEEDS_PROVIDER_SETUP") {
         window.dispatchEvent(
           new CustomEvent("open-setup-center", {
...
**Issue**：[#483](https://github.com/erbanku/CodePilot/issues/483) 阿里云 codingplan 没有 `qwen3.6-plus` 模型。仓库作者 2026-04-15 12:54 UTC 评论"这个还没加，我加一下"。
